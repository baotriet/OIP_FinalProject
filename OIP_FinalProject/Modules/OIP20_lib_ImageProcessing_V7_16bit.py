...
def laplace_sharpen(imgINT, w=0.1, Filter='L4', Threshold=False, TVal=0.1):
    ...
    filtered = (crop_levels(imgINT.astype(np.float) - w * edges.astype(np.float))).astype(np.uint16)
...
def unsharp_mask(imgINT, a = 4, sigma=2.0, tc = 120.):
    ...
    return (crop_levels(imgINT+a*M.astype(np.float))).astype(np.uint16)
...
def hough_circles(imgBIN, Nx, Ny, Nr, K = 5, rmin = 1, rmax = 100):
    ...
    x = nzi[1][:]
    numFGpix = x.shape[0]
    print("Relevant foreground pixels: %i" %(y.shape[0]) )
    ...
    for cntp in range(numFGpix):
        ...
        tmp = (np.square(x[cntp] - XBAR) + np.square(y[cntp] - YBAR) - np.square(RBAR)) < 1
...
def hough_circles2(imgBIN, Nx, Ny, Nr, K = 5, rmin = 1, rmax = 100):
    '''
    Computes the Hough transform to detect circles in the binary 
    image I (of size M × N ), using Nx, Ny, Nr discrete steps for the center coordinates 
    and radius, respectively. Returns the list of parameter triplets (xi, yi, ri) 
...
def SequentialLabeling(imgBIN):
    ...
    # STEP 4 - Relabeling in order from 1 to ...
    mark = 0
    for lbl in R:
        mark += 1
        I[I == np.amin(lbl)] = mark
...
#     ''' manual histogram plot (for uint16 coded intensity images) ''' 
#     fig, ax = plt.subplots()
...
#     ''' manual cumulative histogram plot (for uint16 coded intensity images) '''
#     fig, ax = plt.subplots()
#     ax.set_xlabel('intensity level')
